...
const cachorroErrado = dogs.pop() as string // remove do final do array e retorna o elemento removido | undefined
...
const enderecos1: Array<object | string> = [ ...user.adresses ] // copia o objeto
...
function LogaNome({nome}:LogaNomeProps):void {
    console.log(nome);
}
...
function LogaBixo(props:LogaBixoProps):void {
...
function RetornaTitulo():void {
    const title = document.createElement('h1')
   
    title.innerText =  (document.getElementById('input') as HTMLInputElement).value

    document.getElementById('body')?.appendChild(title)

}

(document.getElementById('button') as HTMLButtonElement).addEventListener('mouseout',RetornaTitulo)
